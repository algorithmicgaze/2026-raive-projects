...
ap.add_argument("clips", type=Path, nargs="+"); ap.add_argument("plate", type=Path); ap.add_argument("out", type=Path)
...
ap.add_argument("--trains", type=Path, default=None, help="output-trains folder; the first --max-trains passages are embedded")
ap.add_argument("--max-trains", type=int, default=2)
...
def pack(d):
    ...
    return {"id": d.name, "lane": m["lane"], "cars": m.get("cars", 1), "frames": fr, "sheet": uri("image/webp", wb)}, len(wb)

# thin lanes first so they are never squeezed out, then the rich ones
dirs = sorted(d for root in args.clips for d in root.glob("lane*/*/meta.json"))
...
if args.trains and (args.trains / "trains.json").exists():
    trains = json.loads((args.trains / "trains.json").read_text())[: args.max_trains]
    out["trainMask"] = uri("image/png", (args.trains / "mask.png").read_bytes())
    out["trains"] = []
    for t in trains:
        data = (args.trains / t["file"]).read_bytes(); total += len(data)
        out["trains"].append({**t, "video": uri("video/mp4", data)})
