...
def main():
    ...
    ap.add_argument("--groups", action="store_true",
                    help="keep groups instead of single cars: merged blobs that stay within one lane and travel bridge to exit")
    args = ap.parse_args()
...
def sane_shape(boxes, lane, h):
    """One vehicle, a lorry or a platoon is never wider than about 1.3
    lanes at its own row. Anything wider is a pack spanning lanes."""
...
def single_car_height(yb):
    """Box height of one car at bottom row yb, fitted on the lane-1 clips."""
    return max(30.0, 0.234 * yb - 52.1)


def estimate_cars(boxes, h):
    """How many cars a group holds: its box height against one car's, at
    the row where the whole group is on screen."""
    best = 1.0
    for x, y, w, bh in boxes:
        if y + bh >= h - 4 or y <= 430:
            continue   # cut by the frame edge or the bridge
        best = max(best, bh / single_car_height(y + bh))
    return int(min(6, max(1, round(best))))
...
def write_clip(t, files, masks, plate, gain, args, src):
    ...
    if len(t.frames) < args.min_len:
        return reject("short")
    if args.groups:
        # groups, and lone trucks (tall, so they count as two or more cars)
        if t.isolated() and estimate_cars(t.boxes, h) < 2:
    ...
    n_cars = estimate_cars(t.boxes, h)
    kind = "car" if n_cars == 1 else ("truck" if t.isolated() else "group")
    meta = {"lane": lane + 1, "source": src, "fps": FPS, "boxes": [], "start_frame": int(t.frames[0]),
            "kind": kind, "cars": n_cars}
    prev_rgba = None
