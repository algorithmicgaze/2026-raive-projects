...
transforms[0] = np.eye(2, 3)
cv2.imwrite(str(args.out / files[0].name), ref, [cv2.IMWRITE_JPEG_QUALITY, 95])
inliers = np.zeros(len(files), int)
for i, f in enumerate(tqdm(files[1:], desc="stabilize"), start=1):
...
print("inliers min/median", inliers[1:].min(), int(np.median(inliers[1:])))
